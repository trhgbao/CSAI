...
class ACO_GraphColoring:
    """
    Giải bài toán tô màu đồ thị bằng thuật toán Đàn kiến (Ant Colony Optimization),
    hỗ trợ chọn thứ tự đỉnh theo DSATUR hoặc cố định.
    (Dòng này chỉ mô tả mục đích lớp, không thay đổi code)
    """
    def __init__(self, adjacency, n_colors, n_ants=10, n_iterations=100,
        ...
        # Nếu người dùng truyền seed: đặt seed cho reproducibility (kết quả có thể lặp lại)
        if seed is not None:
            np.random.seed(seed)
            random.seed(seed)
        ...
        self.neighbors = [np.where(adjacency[i])[0] for i in range(self.n_nodes)]
    ...
    def run(self, verbose=True):
        # degrees dùng cho DSATUR khi cần (tính lại ở đây để đảm bảo có sẵn)
        degrees = np.sum(self.adjacency, axis=1)
            ...
            for ant in range(self.n_ants):
                ...
                for _ in range(self.n_nodes):
                    ...
                    if self.use_dsatur:
                        ...
                        node = self.select_next_node_dsatur(uncolored_idx, saturation, degrees)
                    else:
        ...
        # Trả về lời giải tốt nhất tìm được và số màu tương ứng
